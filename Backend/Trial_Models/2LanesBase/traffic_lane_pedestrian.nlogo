...
to draw-crossing
   ask patches with [(meaning = "road") and (pycor mod 22 = 8 or pycor mod 22 = 9)][
    sprout-crossings 1 [
      set shape "crossing"
      set color white
      set heading 0
...
to make-cars
;  ; num cars doesnt exceed the max amount of room on the road
;  let road-patches patches with [member? pxcor lanes]
;  ;let road-patches patches with [member? pycor lanes] -- for horizontal
;  if number-of-cars > count road-patches [
;    set number-of-cars count road-patches
;  ]

  ;create cars
  ask n-of (number-of-cars) patches with [meaning = "road"] [
