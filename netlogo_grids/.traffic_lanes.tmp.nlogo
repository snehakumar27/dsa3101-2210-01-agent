...
;to create-or-remove-pedestrians
;
;  let road-patches patches with [ member? pycor lanes ]
;  if number-of-pedestrians > count road-patches [
;    set number-of-pedestrians count road-patches
;  ]
;
;  create-turtles (number-of-pedestrians - count turtles) [
;    set color pedestrian-color
;    move-to one-of free road-patches
;    set target-lane pycor
;    set heading 180
;    set top-speed 0.1 + random-float 0.1
;    set speed 0.1
;    ;set patience random max-patience
;  ]
;
  if count turtles > number-of-pedestrians [
    let n count turtles - number-of-pedestrians
    ask n-of n [ other turtles ] of selected-pedestrian [ die ]
  ]
...
to select-pedestrian
  if mouse-down? [
    let mx mouse-xcor
    let my mouse-ycor
    if any? turtles-on patch mx my [
      ask selected-pedestrian [ set color pedestrian-color ]
      set selected-pedestrian one-of turtles-on patch mx my
      ask selected-pedestrian [ set color violet ]
      display
    ]
  ]
end
