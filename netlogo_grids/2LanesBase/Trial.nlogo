...
to create-or-remove-cars
  ...
  let road-patches patches with [ member? pxcor lanes ]
...
to go
  ...
  ask turtles with [ xcor != target-lane ] [ move-to-target-lane ]
  tick
end
...
to move-forward ; turtle procedure
  ...
  let blocking-cars other turtles in-cone (1 + speed) 90 with [ x-distance <= 1 ]
...
to choose-new-lane ; turtle procedure
  ...
  if not empty? other-lanes [
    let min-dist min map [ y -> abs (y - xcor) ] other-lanes
    let closest-lanes filter [ y -> abs (y - xcor) = min-dist ] other-lanes
    set target-lane one-of closest-lanes
    set patience max-patience
  ]
end

;; the following got some issues
to move-to-target-lane ; turtle procedure
  set heading ifelse-value target-lane < xcor [ 0 ] [ 0 ]
  let blocking-cars other turtles in-cone (1 + abs (xcor - target-lane)) 180 with [ y-distance <= 1 ]
  let blocking-car min-one-of blocking-cars [ distance myself ]
  ifelse blocking-car = nobody [
    ...
    ifelse towards blocking-car <= 0 [ slow-down-car ] [ speed-up-car ]
  ]
end

;to move-to-target-lane ; turtle procedure
;  set heading ifelse-value target-lane < ycor [ 180 ] [ 0 ]
;  let blocking-cars other turtles in-cone (1 + abs (xcor - target-lane)) 180 with [ x-distance <= 1 ]
;  let blocking-car min-one-of blocking-cars [ distance myself ]
;  ifelse blocking-car = nobody [
;    forward 0.2
;    set xcor precision ycor 1 ; to avoid floating point errors
;  ] [
;    ; slow down if the car blocking us is behind, otherwise speed up
